...
make_plot_mz = function(mz_dat=NULL, labels=NULL, mz_val=NULL, spot_alpha=NULL){
  ...
  # Make sure all needed columns are present
  need_cols = c('row_id', 'pixel_id', 'x_coord', 'y_coord', 'mz_intx')
  if(sum(colnames(mz_dat) %in% need_cols) == 5){
    mz_dat = mz_dat[, need_cols]
  } else{
  ...
  gg = ggplot2::ggplot() +
    ggplot2::geom_tile(data=mz_dat, ggplot2::aes(x=x_coord, y=y_coord, fill=mz_intx),
                       color=NA,
                       inherit.aes=FALSE) +
    khroma::scale_fill_sunset(name='Normalized\nintensity') +
    ggnewscale::new_scale_fill() +
    ggiraph::geom_tile_interactive(data=mz_dat, ggplot2::aes(x=x_coord, y=y_coord, data_id=row_id, fill=labels),
                                   color=NA,
                                   alpha=spot_alpha,
                                   inherit.aes=FALSE) +

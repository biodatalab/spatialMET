...
#' @export
#'
manual_annotation = function(coords=NULL, intx=NULL, mz=NULL){
  ...
  ui = pageWithSidebar(
    ...
    sidebarPanel(width=3,
                 ...
                 shiny::hr(),
                 sliderInput(inputId="spotSize", label="Pixel sixe [0.1-1]", min=0.1, max=1, value=0.3, step=0.1),
                 shiny::hr(),
...
make_plot = function (spotAlpha, Labels, SpotSize, res, ann, mz){
  ...
  gg = ggplot() +
    ann +
    ggiraph::geom_point_interactive(data=coordinates, aes(x=x, y=y, data_id=id, col=Labels),
                                    #fill=NA, 
                                    shape=0, size=SpotSize, alpha=spotAlpha, inherit.aes=F) +
    ggtitle(mz) +
    coord_fixed() +
    scale_color_discrete(type=col_pal, name='ROIs') +
    ...
    theme(panel.background=element_rect(fill="transparent", colour=NA),
          ...
          legend.text=element_text(angle=30, hjust=1, vjust=1),

...
For researchers working with very large datasets or in high‑performance computing (HPC) environments,
the repository also provides a standalone script `run_pipeline.sh`. This script runs the same preprocessing
and domain‑detection pipeline as the Shiny app, but entirely from the command line – without the need for the graphical interface.
It is ideal for batch processing, integration into existing workflows, or situations where the memory limits of a Docker container are insufficient.

The script uses the same optimised C‑based `hcdist` algorithm as the app, ensuring consistency between command‑line and interactive analyses.
Output files generated by `run_pipeline.sh` can be directly loaded into the Shiny app via the **“Upload preprocessed data”** tab, enabling seamless transition from batch processing to interactive exploration.
